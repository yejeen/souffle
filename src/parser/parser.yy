...
% skeleton "lalr1.cc" % require "3.0.2"
        ...
        define api.value.type variant % define parse.assert %
...
#include "souffle/utility/StringUtil.h"
    ...
    struct Mov<std::vector<A>> {
        ...
        typename std::vector<A>::value_type& operator[](size_t i) {
            return value[i];
        }
        iterator begin() {
            return value.begin();
...
#include "ParserDriver.h"
        ...
        token<std::string> IDENT "identifier" % token<std::string> NUMBER "number" %
        token<std::string> UNSIGNED "unsigned number" % token<std::string> FLOAT "float" %
        ...
        token MAGIC_QUALIFIER "relation qualifier magic" % token TMATCH "match predicate" %
        token TCONTAINS "checks whether substring is contained in a string" %
        token STATEFUL "stateful functor" % token CAT "concatenation of strings" %
        token ORD "ordinal number of a string" % token RANGE "range" % token STRLEN "length of a string" %
        token SUBSTR "sub-string of a string" % token MEAN "mean aggregator" % token MIN "min aggregator" %
        token MAX "max aggregator" % token COUNT "count aggregator" % token SUM "sum aggregator" %
        token TRUE "true literal constraint" % token FALSE "false literal constraint" %
        token PLAN "plan keyword" % token IF ":-" % token DECL "relation declaration" %
        token FUNCTOR "functor declaration" % token INPUT_DECL "input directives declaration" %
        ...
        token TOFLOAT "convert to float" % token TONUMBER "convert to signed integer" %
        token TOSTRING "convert to string" % token TOUNSIGNED "convert to unsigned integer" %
        ...
        token CARET "^" % token PERCENT "%" % token LBRACE "{" % token RBRACE "}" % token SUBTYPE "<:" %
        token LT "<" % token GT ">" % token LE "<=" % token GE ">=" % token NE "!=" % token BW_AND "band" %
        token BW_OR "bor" % token BW_XOR "bxor" % token BW_SHIFT_L "bshl" % token BW_SHIFT_R "bshr" %
        token BW_SHIFT_R_UNSIGNED "bshru" % token BW_NOT "bnot" % token L_AND "land" % token L_OR "lor" %
        token L_XOR "lxor" %
        token L_NOT "lnot"

        /* -- Non-Terminal Types -- */
        % type<Mov<RuleBody>> aggregate_body % type<AggregateOp> aggregate_func %
        type<Mov<Own<AstArgument>>> arg % type<Mov<VecOwn<AstArgument>>> arg_list %
        type<Mov<Own<AstAtom>>> atom % type<Mov<VecOwn<AstAttribute>>> attributes_list %
        type<Mov<RuleBody>> body % type<Mov<Own<AstComponentType>>> comp_type %
        type<Mov<Own<AstComponentInit>>> comp_init % type<Mov<Own<AstComponent>>> component %
        type<Mov<Own<AstComponent>>> component_body % type<Mov<Own<AstComponent>>> component_head %
        type<Mov<RuleBody>> conjunction % type<Mov<Own<AstConstraint>>> constraint %
        type<Mov<RuleBody>> disjunction % type<Mov<Own<AstExecutionOrder>>> exec_order %
        type<Mov<Own<AstExecutionPlan>>> exec_plan % type<Mov<Own<AstExecutionPlan>>> exec_plan_list %
        type<Mov<Own<AstClause>>> fact % type<Mov<std::vector<TypeAttribute>>> functor_arg_type_list %
        type<Mov<std::string>> functor_built_in % type<Mov<Own<AstFunctorDeclaration>>> functor_decl %
        type<Mov<VecOwn<AstAtom>>> head % type<Mov<AstQualifiedName>> identifier %
        type<Mov<VecOwn<AstDirective>>> directive_list % type<Mov<VecOwn<AstDirective>>> directive_head %
        type<AstDirectiveType> directive_head_decl % type<Mov<VecOwn<AstDirective>>> relation_directive_list %
        type<Mov<std::string>> kvp_value % type<Mov<VecOwn<AstArgument>>> non_empty_arg_list %
        type<Mov<Own<AstAttribute>>> attribute % type<Mov<VecOwn<AstAttribute>>> non_empty_attributes %
        ...
        type<Mov<std::vector<std::pair<std::string, std::string>>>> non_empty_key_value_pairs %
        type<Mov<VecOwn<AstRelation>>> non_empty_relation_list % type<Mov<Own<AstPragma>>> pragma %
        type<TypeAttribute> predefined_type % type<Mov<VecOwn<AstAttribute>>> record_type_list %
        type<Mov<VecOwn<AstRelation>>> relation_decl % type<std::set<RelationTag>> relation_tags %
        type<Mov<VecOwn<AstClause>>> rule % type<Mov<VecOwn<AstClause>>> rule_def % type<Mov<RuleBody>> term %
        type<Mov<Own<AstType>>> type % type<Mov<std::vector<AstQualifiedName>>> type_params %
        ...
        /* -- Operator precedence -- */
        % left L_OR % left L_XOR % left L_AND % left BW_OR % left BW_XOR % left BW_AND %
        left BW_SHIFT_L BW_SHIFT_R BW_SHIFT_R_UNSIGNED % left PLUS MINUS % left STAR SLASH PERCENT %
...
/* Top-level statement */
unit : % empty{} | unit directive_head {
...
| unit comp_init {
    driver.addInstantiation($comp_init);
}
| unit pragma {
...
| union_type_list PIPE identifier {
    $$ = $1;
    $$.push_back($identifier);
};

sum_branch_list : sum_branch {
    $$.push_back($sum_branch);
}
| sum_branch_list PIPE sum_branch {
    $$ = $1;
    $$.push_back($sum_branch);
};

sum_branch : IDENT[name] LBRACE RBRACE {
...
relation_decl : DECL non_empty_relation_list attributes_list relation_tags {
...
non_empty_relation_list : IDENT {
    $$.push_back(mk<AstRelation>($1, @1));
}
| non_empty_relation_list COMMA IDENT {
    $$ = $1;
    $$.push_back(mk<AstRelation>($3, @3));
};

/* Attribute definition of a relation */
/* specific wrapper to ensure the err msg says "expected ',' or ')'" */
record_type_list : LBRACKET RBRACKET{} | LBRACKET non_empty_attributes RBRACKET {
    $$ = $2;
};

attributes_list : LPAREN RPAREN{} | LPAREN non_empty_attributes RPAREN {
    $$ = $2;
};

non_empty_attributes : attribute {
    $$.push_back($attribute);
}
...
attribute : IDENT[name] COLON identifier[type] {
    $$ = mk<AstAttribute>($name, $type, @type);
};

/* Relation tags */
relation_tags : % empty{} | relation_tags OUTPUT_QUALIFIER {
...
/* Fact */
fact : atom DOT {
    $$ = mk<AstClause>($atom, Mov<VecOwn<AstLiteral>>{}, nullptr, @$);
};
...
/* Rule definition */
rule_def : head[heads] IF body DOT {
...
exec_order : LPAREN RPAREN {
    $$ = mk<AstExecutionOrder>(AstExecutionOrder::ExecOrder(), @$);
}
| LPAREN non_empty_exec_order_list RPAREN {
    $$ = mk<AstExecutionOrder>($2, @$);
};
...
/* Rule body atom */
atom : identifier LPAREN arg_list RPAREN {
    $$ = mk<AstAtom>($identifier, $arg_list, @$);
};
...
constraint
        ...
        : arg LT arg {
    $$ = mk<AstBinaryConstraint>(BinaryConstraintOp::LT, $1, $3, @$);
}
| arg GT arg {
    $$ = mk<AstBinaryConstraint>(BinaryConstraintOp::GT, $1, $3, @$);
}
| arg LE arg {
    $$ = mk<AstBinaryConstraint>(BinaryConstraintOp::LE, $1, $3, @$);
}
| arg GE arg {
    $$ = mk<AstBinaryConstraint>(BinaryConstraintOp::GE, $1, $3, @$);
}
| arg EQUALS arg {
    $$ = mk<AstBinaryConstraint>(BinaryConstraintOp::EQ, $1, $3, @$);
}
| arg NE arg {
    $$ = mk<AstBinaryConstraint>(BinaryConstraintOp::NE, $1, $3, @$);
}
...
| TRUE {
    $$ = mk<AstBooleanConstraint>(true, @$);
}
| FALSE {
    $$ = mk<AstBooleanConstraint>(false, @$);
};

/* Argument list */
arg_list : % empty{} | non_empty_arg_list {
    $$ = $1;
};
...
/* Atom argument */
arg : STRING {
    $$ = mk<AstStringConstant>($STRING, @$);
}
| FLOAT {
    $$ = mk<AstNumericConstant>($FLOAT, AstNumericConstant::Type::Float, @$);
}
| UNSIGNED {
...
| LBRACKET arg_list RBRACKET {
    $$ = mk<AstRecordInit>($arg_list, @$);
}

// Branch of adt
| DOLLAR IDENT[branch] LPAREN arg_list RPAREN {
    $$ = mk<AstBranchInit>($branch, $arg_list, @$);
}
| DOLLAR IDENT[branch] {
    $$ = mk<AstBranchInit>($branch, VecOwn<AstArgument>{}, @$);
}

| LPAREN arg RPAREN {
    $$ = $2;
}
| AS LPAREN arg COMMA identifier RPAREN {
    $$ = mk<AstTypeCast>($3, $identifier, @$);
}

| AT IDENT LPAREN arg_list RPAREN {
    $$ = mk<AstUserDefinedFunctor>($IDENT, *$arg_list, @$);
}
| functor_built_in LPAREN arg_list RPAREN {
    $$ = mk<AstIntrinsicFunctor>($functor_built_in, *$arg_list, @$);
}
...
| aggregate_func LPAREN arg[first] COMMA non_empty_arg_list[rest] RPAREN {
    ...
    auto agg_2_func = [](AggregateOp op) -> char const* {
        switch (op) {
            ...
            default: fatal("missing base op handler, or got an overload op?");
        }
    };
...
aggregate_body : LBRACE body RBRACE {
    $$ = $body;
}
| atom {
    $$ = RuleBody::atom($atom);
...
component_head : COMPONENT comp_type {
    $$ = mk<AstComponent>();
    $$->setComponentType($comp_type);
}
| component_head COLON comp_type {
    $$ = $1;
    $$->addBaseComponent($comp_type);
}
...
comp_type : IDENT type_params {
    $$ = mk<AstComponentType>($IDENT, $type_params, @$);
};

/* Component type parameters */
type_params : % empty{} | LT type_param_list GT {
    $$ = $type_param_list;
};
...
comp_init : INSTANTIATE IDENT EQUALS comp_type {
    $$ = mk<AstComponentInit>($IDENT, $comp_type, @$);
};
...
/* Functor declaration */
functor_decl : FUNCTOR IDENT LPAREN functor_arg_type_list[args] RPAREN COLON predefined_type {
...
functor_arg_type_list : % empty{} | non_empty_functor_arg_type_list {
    $$ = $1;
};
non_empty_functor_arg_type_list : predefined_type {
    $$.push_back($predefined_type);
}
| non_empty_functor_arg_type_list COMMA predefined_type {
    $$ = $1;
    $$.push_back($predefined_type);
};

/* Predefined type */
predefined_type : IDENT {
    if ($IDENT == "number") {
...
directive_head_decl : INPUT_DECL {
    $$ = AstDirectiveType::input;
}
| OUTPUT_DECL {
    $$ = AstDirectiveType::output;
}
| PRINTSIZE_DECL {
    $$ = AstDirectiveType::printsize;
}
| LIMITSIZE_DECL {
    $$ = AstDirectiveType::limitsize;
};

/* IO directive list */
directive_list : relation_directive_list {
...
relation_directive_list : identifier {
    $$.push_back(mk<AstDirective>(AstDirectiveType::input, $1, @1));
}
| relation_directive_list COMMA identifier {
    $$ = $1;
    $$.push_back(mk<AstDirective>(AstDirectiveType::input, $3, @3));
};
